...
public class AliPayAction extends BaseAction {
	...
	public String saveAliPay() {
		try {
			...
			FuUser fuUser = (FuUser) this.getHttpSession().getAttribute("fuUser");
			fuSmsLogService.saveSendServiceEmail("支付宝充值", "用户" + fuUser.getUserName() + "申请支付宝充值" + total_fee + "元");
			write(sHtmlText);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @return
	 */
	@Action(value = "return_url", results = { @Result(type = "redirect", location = "/user_recharge/successPay.htm") })
