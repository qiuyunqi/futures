package com.hongwei.futures.web.action.admin_op_safe_money;
...
@ParentPackage("admin")
public class AdminOpSafeMoneyAction extends BaseAction {
	private static final long serialVersionUID = 2810508193601045890L;
	...
	public String exportExcel() {
		try {
			// 获取数据库信息
			FuAddMargin addMargin = fuAddMarginService.get(id);
...
//			WritableCellFormat wcf = ExcelUtil.getWritableCellFormatCellFormat();
			...
			wc = ExcelUtil.cloneCellWithValue(
					...
					1,
					"填表时间：  "
							+ DateUtil.getStrFromDate(addMargin.getTime(),
			...
			wc = ExcelUtil.cloneCellWithValue(3, 4, addMargin.getFuUser()
					.getUserName(), wcf5); // 用户姓名
			ws.addCell(wc);
			wc = ws.getWritableCell(4, 4);
			String genderName = "";
			if (null != addMargin.getFuUser().getGender()
					&& 0 == addMargin.getFuUser().getGender()) {
				genderName = "男";
			}
			if (null != addMargin.getFuUser().getGender()
					&& 1 == addMargin.getFuUser().getGender()) {
				genderName = "女";
			}
			...
			wc = ExcelUtil.cloneCellWithValue(
					...
					4,
					"配资比例："
							+ addMargin.getFuProgram().getSafeMoney()
									.divide(new BigDecimal(10000))
							+ ":"
							+ addMargin.getFuProgram().getMatchMoney()
									.divide(new BigDecimal(10000)) + "万", wcf5);
			ws.addCell(wc);
			// 分账户号
			wc = ws.getWritableCell(7, 4);
			wc = ExcelUtil.cloneCellWithValue(7, 4, addMargin.getFuProgram()
			...
			wc = ExcelUtil.cloneCellWithValue(3, 5, addMargin.getFuProgram()
					.getFuServer().getServerRealName(), wcf6); // 期货公司
			ws.addCell(wc);
			// wc = ws.getWritableCell(5, 5);
			// wc = ExcelUtil.cloneCellWithValue(5, 5,
			// addMargin.getFuProgram().getFuUser().getUserName(), wcf6); //开户姓名
			// ws.addCell(wc);
			...
			if (null != addMargin.getMoney()) {
				money = addMargin.getMoney();
			}
			wc = ExcelUtil
					.cloneCellWithValue(1, 8, "账户追加保证金（主账户入    /     万，分账户入"
							+ StringUtil.getDecimalFormat(money) + "元）  ", wcf9); // 客户ID
			ws.addCell(wc);
			...
			String admin_name = "";
			if (null != addMargin.getFuProgram().getFuAdmin()) {
				admin_name = addMargin.getFuProgram().getFuAdmin().getName();
			}
			String checkTime = "";
			if (null != addMargin.getCheckTime()) {
				checkTime = DateUtil.getStrFromDate(addMargin.getCheckTime(),
						"yyyy年MM月dd日");
			}
	 ...
	 * @return
	 */
	@Action("checkSafeMoneyAjax")
	public String checkSafeMoneyAjax() {
		try {
			FuAddMargin addMargin = fuAddMarginService.get(id);
			FuUser user = fuUserService.get(addMargin.getFuUser().getId());
			this.getActionContext().put("addMargin", addMargin);
	 ...
	 * @throws Exception
	 */
	@Action("saveSafeMoneyAjax")
	public String saveSafeMoneyAjax() {
		try {
			FuAddMargin addMargin = fuAddMarginService.get(id);
			FuUser fuUser = fuUserService.get(addMargin.getFuUser().getId());
			if (state == null) {
				write("-2");
				return null;
			} else if (state == 2
				...
				write("-3");
				return null;
			} else if (addMargin.getState() == 2 || addMargin.getState() == 3) {
				write("-4");
				return null;
			} else {
				addMargin.setState(state);
				addMargin.setMoney(money);
				addMargin.setComment(comment);
				addMargin.setCheckTime(new Date());
				addMargin.setFuAdmin(admin);
				if (state == 2) { // 同意
					fuAddMarginService.saveConfirmProfits(addMargin);
				} else if (state == 3) { // 拒绝
				...
				log.setFuUser(addMargin.getFuUser());
				log.setContent(message);
				log.setPrio(1);
				log.setReason("追加保证金");
				log.setDestination(addMargin.getFuUser().getPhone());
