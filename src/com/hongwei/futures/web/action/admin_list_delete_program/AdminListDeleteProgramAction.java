package com.hongwei.futures.web.action.admin_list_delete_program;
...
@ParentPackage("admin")
public class AdminListDeleteProgramAction extends BaseAction{
	@Autowired
	 ...
	 */
	@Action("deleteProgramList")
	public String deleteProgramList() {
		try{
			...
			if(time2!=null)
				map.put("time2", time2);
			if(totalCount==null){
				totalCount=fuProgramService.countDeleteProgram(map);
			}
			List<FuProgram> programList=fuProgramService.findDeleteProgramList((this.getPageNo()-1)*this.getPageSize(), this.getPageSize(), map);
