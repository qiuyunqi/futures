...
public class Code {
	...
	public void init(){
		...
		//  gc.setColor(getRandColor(r,200, 250));
		  ...
		  gc.setColor(getRandColor(r,120, 240));
		  for (int i = 0; i < 100; i++) {
		  ...
		  for (int i = 1; i <= 4; i++) {
		    ...
		    if (i % 2 == 0) {
		      ...
		      gc.drawString(rn.trim(), 13 * i + 6, 16);
		    }
		    else {
		      ...
		      if (index < 26) {
		        ...
		        rs += litter;
		        //将认证码用 drawString 函数显示到图象里
		        gc.drawString(litter.trim(), 13 * i + 6, 16);
		      }
		      else {
		        //当取的下标大于26时，让i--
		        i--;
		  ...
		  this.str = rs;
			...
			try {
				...
				ImageIO.write(pic, "JPEG", imageOut);
				imageOut.close();
