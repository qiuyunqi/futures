...
<div class="center">
    ...
    <div class="fl rt_cont">
        ...
        <div class=" dxgs">
        	<ul>
            	<li class="${empty flag||flag==1?'yxz':''}"><a href="${ctx}/user_program/programList.htm?programType=2&flag=1">进行中</a></li>
                <li class="${empty flag||flag==2?'yxz':''}"><a href="${ctx}/user_program/programList.htm?programType=2&flag=2">待结算</a></li>
                <li class="${empty flag||flag==3?'yxz':''}"><a href="${ctx}/user_program/programList.htm?programType=2&flag=3">已流单</a></li>
           		<li class="${empty flag||flag==4?'yxz':''}"><a href="${ctx}/user_program/programList.htm?programType=2&flag=4">已结束</a></li>
			</ul>
            <div class="clr"></div>
        </div>
        ...
        <div class=" ny_bt">
        	...
        	<span style="color:#076b5e; font-size:12px; float:left">(每月收取管理费)</span>
            <a href="${ctx}/index_guide/applyProgress.htm" class=" gzsm fr">期货月配规则说明</a>
            <div class="clr"></div>
        </div>
        <div class="grglzh_tab" style="margin:13px 20px 20px;*margin-top: -10px">
          ...
          <tbody><tr>
            ...
            <td style="border-bottom:#e3e3e3 solid 1px;" align="right" bgcolor="#f8f8f8" width="120px">风险保证金额(元) </td>
            <td style="border-bottom:#e3e3e3 solid 1px;" align="right" bgcolor="#f8f8f8" width="150px">借款月息(元)  </td>
            <td style="border-bottom:#e3e3e3 solid 1px;" align="center" bgcolor="#f8f8f8" width="120px">借款期限(月)   </td>
            <td style="border-bottom:#e3e3e3 solid 1px;" align="center" bgcolor="#f8f8f8" width="120px">下次付息</td> 
          ...
          <c:forEach items="${programList}" var="pro" varStatus="row">
          		<tr>
		            <td height="35"  style="border-bottom: 0" align="right"><fmt:formatNumber value="${pro.matchMoney}" pattern="#,###,##0.00"/></td>
		            <td align="right" style="border-bottom: 0"><fmt:formatNumber value="${pro.safeMoney}" pattern="#,###,##0.00"/></td>
		            <td align="right" style="border-bottom: 0"><fmt:formatNumber value="${empty pro.manageMoney||pro.manageMoney==0?'免息':pro.manageMoney}" pattern="#,###,##0.00" /></td>
		            <td align="center" style="border-bottom: 0">${pro.cycleNum}个月</td>
		            <td align="center" style="border-bottom: 0">
		            	...
		            	<c:if test="${pro.status==2}">
		            		<fmt:formatDate value="${pro.endManageMoneyTime}" pattern="yyyy-MM-dd"/>
		            	</c:if>
		            </td>
