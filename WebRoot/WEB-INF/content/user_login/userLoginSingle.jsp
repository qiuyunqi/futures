...
<div class=" mfzc_bg">
	...
	<div class=" mfzc_matter">
	  		<div class="userReg">
	  			<a class="userRegZc" href="${ctx}/user_login/ssoauth.htm?action=preLogin&gotoURL=${param.gotoURL}&setCookieURL=${param.setCookieURL}">登录</a>
	  			<a class="logi" href="${ctx}/user_login/ssoauth.htm?action=toRegister&gotoURL=${param.gotoURL}&setCookieURL=${param.setCookieURL}" style="margin-left:-5px;">邀请注册</a>
	  		</div>
	  		<div class="biaodan zhuce_tbl">
	            	<table width="100%" border="0" cellspacing="0" cellpadding="0">
					  <tbody>
					    <tr>
					      <td>
					        <input name="qdBackUrl" value="${qdBackUrl}" type="hidden"/>
...
function toLogin(){
	...
	$.post("${ctx}/user_login/ssoauth.htm",data,function(d){
		var json=eval("("+d+")");
		if(json.code==-1){
		...
		if(json.code==-2){
			$("#accountName").blur();
			jAlert("请输入账号","提示",function(){});
			return null;
		}
		if(json.code==-3){
			$("#password").blur();
			jAlert("请输入密码","提示",function(){});
			return null;
		}
		if(json.code==-4){
			$("#accountName").blur();
			jAlert("登陆账号不存在,请注册","提示",function(){});
			return null;
		} 
		if(json.code==-5){
			$("#accountName").blur();
			jAlert("您输入的密码有误, 请重新输入","提示",function(){});
			return null;
		} 
		if (json.code == 1) {
			location.href = json.setCookieURL;
		}
