...
<div class=" mfzc_bg" id="div1">
	<div class="mfzc_bg_cont">
	  <div class=" mfzc_matter">
	  		<div class="userReg">
	  			<a class="logi" href="${ctx}/user_login/ssoauth.htm?action=preLogin&gotoURL=${param.gotoURL}&setCookieURL=${param.setCookieURL}">登录</a>
	  			<a class="userRegZc" href="${ctx}/user_login/ssoauth.htm?action=toRegister&gotoURL=${param.gotoURL}&setCookieURL=${param.setCookieURL}" style="margin-left:-5px;">邀请注册</a></div>
	...
	function toNext(){
		...
		$.post("${ctx}/user_login/ssoauth.htm",data,function(d){
			...
			if(json){	
				...
				if(json.code==-1){
					jAlert("对不起，您的IP在黑名称中","提示",function(){
			        });
					return false;
				}	
				if(json.code==-13){
