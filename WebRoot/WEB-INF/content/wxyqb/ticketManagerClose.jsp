	...
	<div class="sucContain" >
		<div class="containImg">
			<div class="infoList">
				...
				<a class="giveTcks" href="javascript:void(0);" onclick="deleteTicket(${publish.id})">删除</a>
			</div>
		</div>
	</div>
</body>
...
function deleteTicket(publishId){
	layer.open({
	    content: "确认删除此找劵！",
	    btn: ["确定","取消"],
	    shadeClose: false,
	    yes: function(){
	    	$.post("${ctx}/wxyqb/saveTicketClose.htm",{id: publishId},function(data){
	    		layer.open({
				    content: "删除成功！",
				    btn: ["确定"],
				    shadeClose: false,
				    yes: function(){
				    	layer.closeAll();
						location.href="${ctx}/wxyqb/findTicketManager.htm?userId=${fuUser.id}";
					}
				})
			})
	    }
