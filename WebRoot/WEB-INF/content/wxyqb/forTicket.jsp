		...
		<div class="containImg">
			<div class="infoList">
				 <div class="searchTicket">
		            ...
		            <div class="ticketImg" onclick="searchEvent();"></div>
		        </div>
		        <!-- 列表 -->
...
var searchEvent = function() {
	...
	var query = $("#stockTicket").val();
	if ("" == query || null == query) {
		return false;
	} else {
		$.post("${ctx}/wxyqb/findByCodeOrName.htm", {"query": query}, function(data){
			$(".ticketYqb").remove();
			$(".dropload-down").hide();
	  		var account = data.saList;
	  		...
	  		for (var i=0; i<account.length; i++) {
	      		...
	      		if (account[i]['transactionId'] != null) {
		      		...
		      		html +=	"<td onclick='getInfoById("+account[i]['id']+")'>";
	      		}
     			html += "<div class='tbLft'>"+
				        		"<div class='blueCol siz'>"+account[i]['message']+"</div>"+
				        		"<div class='smallSize'>市值："+account[i]['available']+"</div>"+
				        		"<div class='smallSize'>分配：客户"+account[i]['availableSplit']+"</div>"+
				        		"<div class='soSmaSize'>发布时间："+ account[i]['createTime']+"</div>"+
				     	"</div>"+
				   	"</td>"+
  					"<td onclick='getInfoById("+account[i]['id']+")'><div class='tbRgt'><img src='../images_yqb/jtou.png'/></div></td>";
     			
     		if (account[i]['transactionId'] != null) {
      			html += "<td class='grayBg bigSz whiteCol'><span>已抢</span></td>";
      		} else if(${sessionScope.fuUser != null}) {
      			if (${sessionScope.fuUser.isTransaction == 1}) {
          			html += "<td onclick='getInfoById("+account[i]['id']+")' class='redBg bigSz whiteCol'><span>抢</span></td>";
          		} else {
	      			html += "<td onclick='getInfoById("+account[i]['id']+")' class='wRob bigSz whiteCol'><span>未抢</span></td>";
          		}	
      		} else {
	      		html += "<td onclick='getInfoById("+account[i]['id']+")' class='wRob bigSz whiteCol'><span>未抢</span></td>";
      		}
...
$(function() {
    ...
    $( "#stockTicket" ).autocomplete({
      ...
      select: function( event, ui ) {
    	...
    	$("#ui-id-1").hide();
    	$(".ui-helper-hidden-accessible").hide();
        $.post("${ctx}/wxyqb/findByCodeOrName.htm", {"query": $("#stockTicket" ).val()}, function(data) {
        	$(".ticketYqb").remove();
			$(".dropload-down").hide();
	  		var account = data.saList;
	  		var html = "<table class='ticketYqb' cellpadding='0' cellspacing='0' width='100%' border='0'>";
    ...
    .data( "ui-autocomplete" )._renderItem = function( ul, item ) {
        return $( "<li>" )
          .append(  "<a>" + "<span>"+ item.category + "</span></a>")
          .appendTo( ul );
      };
  });
</script>
        ...
        loadUpFn : function(me) {
        	$.ajax({
   	         ...
   	         success: function(dt){
         		...
         		counter++;
         		$(".ticketYqb").remove();
   		      	html += "<table class='ticketYqb' cellpadding='0' cellspacing='0' width='100%' border='0'>";
         		for (var i = 0; i < d.length; i++) {
         			html += "<tr>";	
	   		      	if (d[i]['transactionId'] != null) {
	   		      		html +=	"<td>";
	   	      		} else {
	   		      		html +=	"<td onclick='getInfoById("+d[i]['id']+")'>";
	   	      		}
   		        		html += "<div class='tbLft'>"+
   					        		"<div class='blueCol siz'>"+d[i]['message']+"</div>"+
   					        		"<div class='smallSize'>市值："+d[i]['available']+"</div>"+
   					        		"<div class='smallSize'>分配："+d[i]['availableSplit'] +"</div>"+
   					        		"<div class='soSmaSize'>发布时间："+ d[i]['createTime']+"</div>"+
   		        				"</div>"+
   		      			"</td>";
   		 	      	if (d.transactionId != null) {
   		   	      		html +=	"<td><div class='tbRgt'><img src='../images_yqb/jtou.png'/></div></td>";
   		   	      	} else {
   		   	      		html +=	"<td onclick='getInfoById("+d[i]['id']+")'><div class='tbRgt'><img src='../images_yqb/jtou.png'/></div></td>";
   		   	      	}
   		      		if (d[i]['transactionId'] != null) {
   		      			html += "<td class='grayBg bigSz whiteCol'><span>已抢</span></td>";
   		      		} else if(${sessionScope.fuUser != null}) {
	   		      		if (${sessionScope.fuUser.isTransaction == 1}) {
	   		      			html += "<td onclick='getInfoById("+d[i]['id']+")' class='redBg bigSz whiteCol'><span>抢</span></td>";
	   		      		} else {
