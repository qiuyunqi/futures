...
.zhang{background:#fff;border-right:1px solid #ededed;}
.die{background:#fff;}
.activ{border-bottom:2px solid #004e99;}
...
.coordinate_x{border-right:1px solid #363636;width: 15%;position: relative;float:left;}
.coordinate_xUl li span{float:right;}
.coordinate_xUl li{height: 40px;line-height: 1px;}
.coordinate_xUl li:last-child{height: 10px;}
.coordinate_x .upJt{position: absolute;right: -5px;bottom:-5px;}
.coordinate_yUl li{height:39px;line-height:39px;border-bottom:1px solid #fff;}
.coordinate_y{width:84%;margin-left: 15%;position: relative;}
.coordinate_y .rgtJt{position: absolute;right:0;top:-9px;}
.coordinate_yUl {margin-top:20px;border-top:1px solid #363636;border-left: 1px solid #383838;}
.coordinate_ybg{color:#fff;text-align: right;}
.botmTit{width:84%;margin-left: 15%;float:right;text-align: right;position: absolute;top:-20px;}
.botmTitFir{width:50%;float:left;}
.botmTitSec{width:24%;float:left;}
 .botmTitThi{width:20%;float:left;}
 .xuxian{position: absolute;z-index:99;top: 0;width:100%;height:370px;}
...
<body onmousewheel="return false;">
	 <div class="app-wrapper" id="wrapper">
        <div class="app-content" >
       		<div class="infoBtn"><a class="zhang activ" href="javascript:void(0);">看涨详情</a><a class="die" href="javascript:void(0);">看跌详情</a></div>
        	<div class="clear"></div>
        	<!-- 看涨-->
        	<div class="wqqUp">
        		<div class="ctxName">
        	...
        	<div class="wqqDown">
	        	<div class="ctxName">
	        		<p><span>产品名称：</span>${contents.name }</p>
	        		<p><span>申购时间：</span>${downDetail.buyBeginTime}至${downDetail.buyEndTime}中午12点</p>
	        		<p><span>封闭时间：</span>${downDetail.closeBeginTime}至${downDetail.closeEndTime}</p>
	        		<p><span>加入条件：</span>10元起购，以10元的倍数递增</p>
	        	</div>
	        	<div class="infoList">
	        		<span>产品简介</span>
	        		<p>本产品与${contents.OOI}挂钩，随${contents.OOI}下跌获得收益。一切以最终跌幅（${downDetail.closeEndTime}收盘价）为准，与封闭期内涨跌幅无关。</p>
	        	</div>
	        	<div class="infoList">
	        		<span>跌幅公式</span>
	        		<p>（${downDetail.closeEndTime}收盘价-${downDetail.buyEndTime}收盘价）/${downDetail.closeEndTime}收盘价，负数为下跌。</p>
	        	</div>
	        	<div class="infoList sysm">
	        		<span>收益说明</span>
	        		<p>1.跌幅对应的收益为下表所示${downDetail.down_dw_num}档，跌幅最终达到哪一档，用户既可以获得对应收益，最高可获得高达${downDetail.down_max_bs}倍投资本金的净回报！</p>
	        		<p>2.最终跌幅超过${downDetail.down_max_zf}%时，向用户返还本息合计${downDetail.down_dw9_val}%。</p>
	        		<p>3.最终跌幅小于${downDetail.down_min_zf}%时，客户损失全部投资本金。</p>
	        		<div class="coordinateAll">
	        			<h5>${contents.OOI}跌幅/收益（${downDetail.closeEndTime}-${downDetail.buyEndTime}）</h5>
	        			<div class="coordinate">
	        				<div class="coordinate_x">
	        					<ul class="coordinate_xUl">
	        						<li>${downDetail.down_dw1}${empty downDetail.down_dw1?'':'%'}<span>-</span></li>
	        						<li>${downDetail.down_dw2}${empty downDetail.down_dw2?'':'%'}<span>-</span></li>
	        						<li>${downDetail.down_dw3}${empty downDetail.down_dw3?'':'%'}<span>-</span></li>
	        						<li>${downDetail.down_dw4}${empty downDetail.down_dw4?'':'%'}<span>-</span></li>
	        						<li>${downDetail.down_dw5}${empty downDetail.down_dw5?'':'%'}<span>-</span></li>
	        						<li>${downDetail.down_dw6}${empty downDetail.down_dw6?'':'%'}<span>-</span></li>
	        						<li>${downDetail.down_dw7}${empty downDetail.down_dw7?'':'%'}<span>-</span></li>
	        						<li>${downDetail.down_dw8}${empty downDetail.down_dw8?'':'%'}<span>-</span></li>
	        						<li>${downDetail.down_dw9}${empty downDetail.down_dw9?'':'%'}</li>
	        						<li>跌幅</li>
	        					</ul>
	        					<span class="upJt">∨</span>
	        				</div>
	        				...
	        				<div class="coordinate_y">
	        					...
	        					<ul class="coordinate_yUl">
	        						<li class="coordinate_ybg egt"><span class="gren"></span><span class="coordinate_yVal">${downDetail.down_dw1_val}</span>%</li>
	        						<li class="coordinate_ybg egt"><span class="gren"></span><span class="coordinate_yVal">${downDetail.down_dw2_val}</span>%</li>
	        						<li class="coordinate_ybg sixd"><span class="coordinate_yVal">${downDetail.down_dw3_val}</span>%</li>
	        						<li class="coordinate_ybg seve"><span class="coordinate_yVal">${downDetail.down_dw4_val}</span>%</li>
	        						<li class="coordinate_ybg frt"><span class="coordinate_yVal">${downDetail.down_dw5_val}</span>%</li>
	        						<li class="coordinate_ybg sed"><span class="coordinate_yVal">${downDetail.down_dw6_val}</span>%</li>
	        						<li class="coordinate_ybg thd"><span class="coordinate_yVal">${downDetail.down_dw7_val}</span>%</li>
	        						<li class="coordinate_ybg fort"><span class="coordinate_yVal">${downDetail.down_dw8_val}</span>%</li>
	        						<li class="coordinate_ybg fve"><span class="coordinate_yVal">${downDetail.down_dw9_val}</span>%</li>
	        					</ul>
	        				</div>
	        			</div>
	        		</div>
	        	...
	        	<div class="infoList">
	        		<span>结算说明</span>
	        		<p>${downDetail.clearTime}完成所有预算，向用户返还投资收益。</p>
	        	</div>
	        </div>
       </div>
   </div>
...
$(".wqqDown").hide();
	...
	$(".zhang").click(function(){
		$(".wqqUp").show();
		$(".wqqDown").hide();
		$(".zhang").addClass("activ");
		$(".die").removeClass("activ");
	});
