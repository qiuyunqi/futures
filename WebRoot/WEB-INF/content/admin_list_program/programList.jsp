...
<div class="content">
    <div class=" rt_cont">
    	<div class="rt_cont_mat">
    	    <div class="rt_cont_title">方案管理<domi:privilege url="/admin_list_program/exportExcelAjax.htm"><a href="javascript:void(0);" onclick="exportInfo();" class="fr add">导出</a></domi:privilege></div>
            <div class="form">
            <form id="searchForm" action="${ctx}/admin_list_program/programList.htm" method="post">
				    ...
				    <td>
				        <div class="form_cont">
				            ...
				            <div class="fl select"><select style="width:178px;" id="status" name="status">
				            	<option value="">全部</option>
				            	<option <c:if test="${status==-2}">selected="selected"</c:if> value="-2">已删除</option>
				            	<option <c:if test="${status==-1}">selected="selected"</c:if> value="-1">已关闭</option>
				            	...
				            	<option <c:if test="${status==3}">selected="selected"</c:if> value="3">拒绝开户</option>
				            	...
				            	<option <c:if test="${status==6}">selected="selected"</c:if> value="6">结算中</option>
				            	<option <c:if test="${status==7}">selected="selected"</c:if> value="7">已穿仓</option>
				            </select></div>
				            <div class="clr"></div>
				        </div>
				    </td>
				  ...
				  <tr>
				    ...
				    <td>
				        <div class="form_cont">
				            ...
				            <div class="fl"><a href="javascript:void(0);" class="search" onclick="searchInfo();">搜索</a><a href="${ctx}/admin_list_program/programList.htm" class="remove" style="margin-left:30px;">清空</a></div>
				            <div class="clr"></div>
				        </div>
				    </td>
				  </tr>
          ...
          <div class=" yhlb_title">方案列表</div>
            	 ...
            	 <table width="100%">
				  <tbody>
					...
					<c:forEach items="${programList}" var="program" varStatus="row">
						    ...
						    <td style="color:#137490" <domi:privilege url="/admin_op_program/programListDetail.htm">onclick="window.location='${ctx}/admin_op_program/programListDetail.htm?id=${program.id}'"</domi:privilege> >${program.id}</td>
						    <td style="color:#137490" <c:if test="${program.addMatchId>0||program.subMatchId>0}">onclick="window.location='${ctx}/admin_op_program/programListDetail.htm?id=${program.addMatchId>0?program.addMatchId:program.subMatchId}'"</c:if>>${program.addMatchId>0?program.addMatchId:program.subMatchId>0?program.subMatchId:'---'}${program.addMatchId>0?'（增配）':program.subMatchId>0?'（减配）':''}</td>
						    ...
						    <td><c:if test="${program.programType==1}">${program.moneyPercent}</c:if><c:if test="${program.programType==2}">${program.moneyPercent*100}%</c:if></td>
						  	...
						  	<!-- <td>---</td> -->
						  	<td><c:if test="${empty program.tradeTime}">---</c:if><c:if test="${!empty program.tradeTime}"><fmt:formatDate value="${program.tradeTime}" pattern="yyyy-MM-dd"/></c:if>
						  	...
						  	<domi:privilege url="/admin_op_program/programListDetail.htm">
						  	<a href="${ctx}/admin_op_program/programListDetail.htm?id=${program.id}">查看详情</a>
						  	</domi:privilege>
						  	</div></td>
						  </tr>
					  </c:forEach>  
           ...
           <div class="page">
				<domi:pagination ctx="${ctx}" pageVolume="${pageSize}" url="${ctx}/admin_list_program/programList.htm?totalCount=${totalCount}" totalNum="${totalCount}" curPageNum="${pageNo}" formId="pageForm" >
