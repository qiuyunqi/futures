	...
	function saveInfo(){
	    jConfirm("确定结算方案？","操作提示",function(res){
			if(res){
				...
				$.post("${ctx}/admin_op_program/closeProgramOfflineAjax.htm?id=${id}",data,function(d){
					var json=eval("("+d+")");
					if(d==-1){
					...
					$.alerts.okButton="确定";
				    jAlert("结算成功，请立即给交易账户线下结算！","提示",function(){
						location.href="${ctx}/admin_list_over_program/offlineOverProgramList.htm";
				    });
				});
			}
		});
