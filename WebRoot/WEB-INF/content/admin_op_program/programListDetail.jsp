...
<c:set var="second" value="14"/>
<div class="content">
    <div class=" rt_cont">
    	<div class="rt_cont_mat">
    	    <div class="rt_cont_title">方案管理详情<a href="${ctx}/admin_list_program/programList.htm" class="fr add">返回</a></div>
          ...
          <div class=" yhlb_title" style="text-align: center;">《投顾合同要素表》--${program.programType==1?'期货日配':program.programType==2?'期货月配':'期货大赛'}（${program.status==-2?'已删除':program.status==-1?'已关闭':program.status==0?'待审核':program.status==1?'审核中':program.status==2?'交易中':program.status==3?'拒绝开户':program.status==4?'等待结算':program.status==5?'交易结束':program.status==6?'结算中':'已穿仓'}）</div>
            <div class="yhlb">
          	...
          	<div class=" yhlb_title">增减配主方案记录</div>
	            <div class="yhlb">
	            	 <table width="100%">
					  <tbody>
					  <tr>
					    ...
					    <th>实盘资金</th>
					    <th>风险保证金</th>
					    <th>亏损预警线（增配：合并后，减配：原方案）</th>
					    <th>亏损平仓线（增配：合并后，减配：原方案）</th>
					    <th>状态</th>
					    <th>开户审核人</th>
					    <th>开户时间</th>
					    <th>结束时间</th>
					  </tr>
					  <c:if test="${program.addMatchId>0}">
					  		<tr style="cursor: pointer;" onmouseover="$(this).css('background','#f6f6f6');" onmouseout="$(this).css('background','');">
							    ...
							    <td>${addProgram.id}_增配主方案</td>
							    <td><fmt:formatNumber value="${empty addProgram.matchMoney?0:addProgram.matchMoney}" pattern="#,###,##0.00"/></td>
							    <td><fmt:formatNumber value="${empty addProgram.safeMoney?0:addProgram.safeMoney}" pattern="#,###,##0.00"/></td>
							    <td><fmt:formatNumber value="${warnLine}" pattern="#,###,##0.00"/></td>
							    <td><fmt:formatNumber value="${closeLine}" pattern="#,###,##0.00"/></td>
							    <td>${addProgram.status==-2?'已删除':addProgram.status==-1?'已关闭':addProgram.status==0?'待审核':addProgram.status==1?'审核中':addProgram.status==2?'交易中':addProgram.status==3?'拒绝开户':addProgram.status==4?'等待结算':addProgram.status==5?'交易结束':addProgram.status==6?'结算中':'已穿仓'}</td>
							    <td>${empty addProgram.fuAdmin.name?'----':addProgram.fuAdmin.name}</td>
							    <td><fmt:formatDate value="${addProgram.checkTime}" pattern="yyyy-MM-dd HH:mm:ss"/>${empty addProgram.checkTime?'----':''}</td>
							    <td><fmt:formatDate value="${addProgram.closeTime}" pattern="yyyy-MM-dd HH:mm:ss"/>${empty addProgram.closeTime?'----':''}</td>
							 </tr>
					  </c:if>
					  <c:if test="${program.subMatchId>0}">
					  		<tr style="cursor: pointer;" onmouseover="$(this).css('background','#f6f6f6');" onmouseout="$(this).css('background','');">
							    <td align="center" class="num">${(row.index+1)+((pageNo-1)*pageSize)}</td>
							    <td>${subProgram.id}_减配主方案</td>
							    <td><fmt:formatNumber value="${empty subProgram.matchMoney?0:subProgram.matchMoney}" pattern="#,###,##0.00"/></td>
							    <td><fmt:formatNumber value="${empty subProgram.safeMoney?0:subProgram.safeMoney}" pattern="#,###,##0.00"/></td>
							    <td><fmt:formatNumber value="${warnLine}" pattern="#,###,##0.00"/></td>
							    <td><fmt:formatNumber value="${closeLine}" pattern="#,###,##0.00"/></td>
							    <td>${subProgram.status==-2?'已删除':subProgram.status==-1?'已关闭':subProgram.status==0?'待审核':subProgram.status==1?'审核中':subProgram.status==2?'交易中':subProgram.status==3?'拒绝开户':subProgram.status==4?'等待结算':subProgram.status==5?'交易结束':subProgram.status==6?'结算中':'已穿仓'}</td>
							    <td>${empty subProgram.fuAdmin.name?'----':subProgram.fuAdmin.name}</td>
							    <td><fmt:formatDate value="${subProgram.checkTime}" pattern="yyyy-MM-dd HH:mm:ss"/>${empty subProgram.checkTime?'----':''}</td>
							    <td><fmt:formatDate value="${subProgram.closeTime}" pattern="yyyy-MM-dd HH:mm:ss"/>${empty subProgram.closeTime?'----':''}</td>
							 </tr>
					  </c:if>
					 <c:if test="${empty addProgram && empty subProgram}">
					    <tr>
						  <td colspan="10">
				        	<div class=" empty0"><img src="${ctx}/images_qhht/empty_03.jpg" width="93" height="63"><p class="empty0_font">您暂时还没有增减配记录</p></div>
				          </td>
					    </tr>
					  </c:if>
					</tbody>
          ...
          <div class=" yhlb_title">不良记录</div>
	            <div class="yhlb">
					 ...
					 <c:if test="${empty badList}">
					    <tr>
						  <td colspan="14">
				        	<div class=" empty0"><img src="${ctx}/images_qhht/empty_03.jpg" width="93" height="63"><p class="empty0_font">您暂时还没有不良记录</p></div>
				          </td>
					    </tr>
					  </c:if>
					</tbody>
    ...
    <div class=" bottom">
	    <a href="${ctx}/admin_list_program/programList.htm" class="exit fr">关闭</a>
	    <c:if test="${program.status==2}">
	    <a href="javascript:void(0);" onclick="tellOpenProgram(${program.id});" class="exit fr">告知开户</a>
	    </c:if>
		<%-- <a href="javascript:void(0);" onclick="updateProgram(${program.id});" class="exit fr">修改要素表</a>
	    <a href="javascript:void(0);" onclick="addBadRecord();" class="exit fr">加不良记录</a> --%>
