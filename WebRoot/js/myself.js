//确定型
function sureInfo(buttonValue,tipContent,tipTitle){
	$.alerts.okButton=buttonValue;
	jAlert(tipContent,tipTitle);
}
//确定/取消型
function sureAndCancle(tipContent,tipTitle){
	jConfirm(tipContent, tipTitle,function(res){//alert(res);
	});
}
//输入型
function inputInfo(text,tipTitle,value){
	jPrompt(text, tipTitle, value);
	//jPrompt('请输入你要设定的值：', '请输入', '20');
}
//嵌入页面型
function toInner(url,tipTitle,width,height){
	jIframe(url, tipTitle,width,height);
	//jIframe('http://www.luocheng.cn/', '老奔博客','460','150');
}
//选择交互型
function addNewHtml(html,tipTitle,width,height){
	...
	*/
	jChoose(html, tipTitle,width,height,function(res){
		if(res){
			alert(res.html());
		}
	});
...
//HTML片段型
function addHtml(html,tipTitle,width,height){
	jHtml(html, tipTitle,width,height);
